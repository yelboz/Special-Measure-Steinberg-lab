...
switch nargin
   ...
   smaux.smgui.nullpanel=uipanel('Parent',smaux.smgui.figure1,...
    ...
    %UI Controls for plot selection and clearing
    ...
    smaux.smgui.clear_plot=uicontrol('Parent',smaux.smgui.nullpanel,'Style','pushbutton',...
        'String','Clear Plot Selections',...
        'FontSize',10,...
        'Position',[200 325 150 20],...
        'Callback',@ClearPlot);
...
function UpdateConstants(varargin)
    ...
    if exist('smscan.consts.setchan','var')
...
%clears plot choices, clear_plot pushbutton callback
function ClearPlot(varargin)
    global smaux;
    set(smaux.smgui.oneDplot_lbh,'Value',[]);
    set(smaux.smgui.twoDplot_lbh,'Value',[]);
end
...
function makeloopgetchans(i)
    ...
    if size(smaux.smgui.loopvars_getchans_pmh,1)>=i
        smaux.smgui.loopvars_getchans_pmh(i,:)=[];
    end
    channelnames = {};
    for k = 1:length(smdata.channels)
        channelnames{k}=smdata.channels(k).name;
