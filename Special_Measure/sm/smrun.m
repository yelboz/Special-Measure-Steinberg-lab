...
for i=1:length(scandef)
    if isfield(scandef(i),'setchanranges') && ~isempty(scandef(i).setchanranges) %&& ~isempty(scandef(i).setchanranges{1})
...
if nargin >= 2
    ...
    save(filename, 'configvals', 'configdata', 'scan', 'configch');
        ...
        display('failed to save in smrun, 444')
    end
    ...
    for j = loops(~isdummy(loops))
        ...
        for k = 1:length(scandef(j).procfn) 
            ...
            if isfield(scandef(j).procfn(k).fn, 'outdata')
                ...
                if isempty(fn)
                    temp_data = subsasgn(data{ind + k}, s, newdata{k});
                    data{ind + k} = temp_data; 
                    clear temp_data;
                    % modified Topology
                else
                    temp_data = subsasgn(data{ind + k}, s, newdata{fn.outchan(1)});
                    data{ind + k} = temp_data;
                    clear temp_data;
                    % modified Topology
                end
        ...
        if j == scan.saveloop(1) && ~mod(count(j), scan.saveloop(2)) && nargin >= 2
            ...
            save(filename, '-append', 'data');
                ...
                display('failed to save in smrun, 617')
            end
        end
    ...
    if get(figurenumber, 'CurrentCharacter') == char(27)
        ...
        if nargin >= 2
            ...
            save(filename, 'configvals', 'configdata', 'scan', 'configch', 'data')
                ...
                display('failed to save in smrun, 647')
            end
        end
        set(figurenumber, 'CurrentCharacter', char(0));
...
if nargin >= 2
    ...
    save(filename, 'configvals', 'configdata', 'scan', 'configch', 'data')
            ...
            display('failed to save in smrun, 672')
    end
end
end
