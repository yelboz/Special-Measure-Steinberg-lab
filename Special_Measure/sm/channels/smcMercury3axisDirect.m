...
%channel 1: FIELD
%disp(ico)
%maginst = tcpip('140.247.189.160',7020)

% Added 2015/11/20:

% From scratch, connect to IPS using Oxford VRM software.  
% Then use 'netstat -an' on cmd to determine the open port (should be
% 7020).
% Then define a TCPIP object as above.

% THESE LIMITS ARE FOR THE 12-3-1 MAGNET IN THE MX400 CRYOSTAT!
...
% 1 if in 1T sphere
% 2 if in 3T circle
% 3 if in 1T sphere and 3T circle
% 4 if in 12T cylinder
% 5 if in 1T sphere and 12T cylinder
% 6 if in 3T circle and 12T cylinder
% 7 if in all
...
if (abs(B(3))<=12 && norm(B(1:2))<=.523)
    bool=bool+4;
end
if (abs(B(2))<=0.13 && norm([B(1) B(3)])<=3)
    bool=bool+2;
end
end
