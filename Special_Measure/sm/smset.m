...
if ~isempty(rampchan)
    chantest = channels(ramptime==max(ramptime));
    chantestmarker = 0;
    for k = 1:size(chantest,1)
        if ~(strcmp(smdata.channels(chantest(k)).name,'Bx')||...
         strcmp(smdata.channels(chantest(k)).name,'By')||...
         strcmp(smdata.channels(chantest(k)).name,'Bz'))
            chantestmarker = chantestmarker + 1;
        end
    end
    if chantestmarker>0 
